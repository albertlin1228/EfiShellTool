...
ifeq ($(TOOL_CHAIN_FAMILY),MSFT)
  ...
  ifneq ($(INTEL_ARCHITECTURE_SUPPORT),1)
    $(error Active toolchain does not support AARCH64 architecture. Update TOOL_CHAIN_TAG token to change the active tool chain)
  endif

  ifeq ($(TOOL_CHAIN_TAG),VS2015)
   include $(CONFIGURATION_DIR)VS2015.inc.mak
  else ifeq ($(TOOL_CHAIN_TAG),VS2019)
   include $(CONFIGURATION_DIR)VS2019.inc.mak
  else
    $(error Unsupported TOOL_CHAIN_TAG $(TOOL_CHAIN_TAG))
  endif
